...
functions {
  ...
  real discrete_seir_S(int N, real E0, real gamma, real beta, real alpha, int last_time, vector social_distance, int step_size){
...
data {
  ...
  // speculation
  real<lower=intervention_time> future_intervention;
  real<lower=0, upper=1> future_social;
}
...
transformed parameters{
  ...
  vector<lower=0, upper=1>[last_time+116] social_distance;
  vector<lower=0>[last_time] newly_exposed;
  
  for (t in 1:(last_time+116)){
...
generated quantities {
  ...
  vector<lower=0>[last_time+116] social_none;
  vector<lower=0>[last_time+116] social_more;
  
  vector<lower=0>[last_time+90] projected_none;
  vector<lower=0>[last_time+90] projected_more;
  vector<lower=0>[last_time+90] projected_never;
  ...
  // projections for more and less social distancing
  for (t in 1:(last_time+116)){
    social_none[t] = social_distance[t] * (1.0 + (1.0/social - 1.0) * inv_logit((t-future_intervention)/2.0));
    social_more[t] = social_distance[t] * (1 - (1-future_social) * inv_logit((t-future_intervention)/2.0));
  }
  
  projected_none =  discrete_seir(N, E0, gamma, beta, alpha, last_time+90, social_none, step_size);
  projected_more =  discrete_seir(N, E0, gamma, beta, alpha, last_time+90, social_more, step_size);
  projected_never =  discrete_seir(N, E0, gamma, beta, alpha, last_time+90, rep_vector(1.0, last_time+90), step_size);
  
  E_current = N-discrete_seir_S(N, E0, gamma, beta, alpha, last_time+116, social_distance, step_size); 
  E_more = N-discrete_seir_S(N, E0, gamma, beta, alpha, last_time+116, social_more, step_size); 
  E_none = N-discrete_seir_S(N, E0, gamma, beta, alpha, last_time+116, social_none, step_size); 
}
